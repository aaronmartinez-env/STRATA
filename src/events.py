...
Fix applied (real-data validation):
  - Window increased from 3 to 7 days, min_periods 2->3 (3-day window produced
    zero calima detections on real 2024 data — baseline never stabilised)
  - Added absolute PM10 threshold fallback (>50 µg/m³) so extreme events are
    caught even when the rolling baseline is suppressed by data gaps
  - DANA detection has its own independent PM10 criterion, not dependent on
    calima's pm10_spike column, so it fires even when calima logic doesn't
"""
