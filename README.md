...
STRATA is moving from a static portfolio file to a deployed web app at `strata-atmos.com`, with two modules:

- **Live** — interactive exploration of RVVCCA hourly data via a FastAPI proxy backend (any station, any date range, 2009–present)
- **Case Studies** — the existing 2021–2022 and 2024 DANA analyses, ported into the new frontend

The frontend lives in [`frontend/`](frontend/) (Vite + React Router). Run it with:
...
The Python pipeline in `src/` is unchanged and remains the source of truth for all findings.
